...
int write_paras_vpvs(vector<paradef> paras, char *outname, char *outdir, int invtype)
{
  // HVL: new function to write out the data with the perturbation of vpvs 
  // 20261030
    FILE *ff;
    char nameparas[300];
    ...
    if (ff == NULL)
    {
        fprintf(stderr, "Cannot open para debug output file: %s\n", nameparas);
        return 0;
    }

    for (int i = 0; i < (int)paras.size(); i++)
    {
        int idx_tt0   = paras[i].nparav;
        int idx_tr0   = paras[i].nparav + paras[i].npara_tt;
        int idx_vpvs0 = paras[i].nparav + paras[i].npara_tt + paras[i].npara_tr;

        fprintf(ff,
            "# model %d | npara=%d | nparav=%d | npara_tt=%d | npara_tr=%d | npara_vpvs=%d\n",
            i,
            paras[i].npara,
            paras[i].nparav,
            paras[i].npara_tt,
        ...
        for (int j = 0; j < paras[i].npara; j++)
        {
            ...
            fprintf(ff, "%04d %-16s %.10g\n", j, ptype, paras[i].parameter[j]);
        }
...
vector<int> vmono, vector<int> vgrad, double pp, int flag_misfit, int invtype)
{
  ...
  if (flag_misfit > 0)
  {
    // int idx_vpvs0 = tpara.nparav + tpara.npara_tt + tpara.npara_tr;

    // fprintf(stderr, "\n[BEFORE get_misfit1]\n");
    // for (int jj = idx_vpvs0; jj < idx_vpvs0 + tpara.npara_vpvs; jj++)
    // {
    //     fprintf(stderr, "  tpara[%03d]=%g\n", jj, tpara.parameter[jj]);
    // }
    // paradef tpara_keep = tpara; // copy for safety

    get_misfit1(tpara, ttmodel, pp, invtype);

    // tpara = tpara_keep; // take back to advoid the reset of the vpvs parameters
...
// ============================= Print to debug ==========================================================
  ...
  // fprintf(stderr,
  //     "\n[get_new_para_misfit] final tpara before assign | npara=%d | idx_vpvs0=%d | npara_vpvs=%d\n",
  //     tpara.npara, idx_vpvs0, tpara.npara_vpvs
  // );

  // for (int jj = idx_vpvs0; jj < idx_vpvs0 + tpara.npara_vpvs; jj++)
  // {
  //     fprintf(stderr,
  //         "  tpara VpVs[%03d] = %g | min=%g | max=%g\n",
  //         jj,
  //         tpara.parameter[jj],
  //         tpara.space1[jj][0],
  //         tpara.space1[jj][1]
...
int write_paras(FILE *outf, int i1, int i2, int i3,
                paradef &para1, modeldef &ttmodel,
                double newL, double newmisfit, int id_thread)
{
    // Write the output measurement parameter for running - use later as threshold to choose the model
    int j;
    // fprintf(stderr, "write_paras: outf=%s | npara=%d\n", outf, para1.npara);
    // sleep(2);
    if (outf == NULL)
    {
    ...
    // fprintf(stderr, "write_paras VpVs check:\n");
    // for (j = 12; j < para1.npara; j++)
    // {
    //     fprintf(stderr, "  para[%03d] = %g\n", j, para1.parameter[j]);
    // }
    // usleep(1000000);
    ...
    {
        fprintf(outf,"%g ", para1.parameter[j]);
    }
    ...
    fprintf(outf,"\n > %g %g %g %g %g %g %g %g %g\n",
        ...
        newmisfit,
        ttmodel.data.rf.L,
        ttmodel.data.rf.misfit,
        ttmodel.data.disp.L,
        ttmodel.data.disp.misfit,
        ttmodel.data.disp.pmisfit,
        ttmodel.data.disp.gmisfit,
        ttmodel.data.disp.emisfit
    );
...
std::vector<boost::array<double, 10> > & infos, int invtype)
{
  ...
  for (i=0;i<jumpn;i++) {
     ...
     #pragma omp critical(write_para_p3)
       ...
       if (fmod(double(iaccp),100.)==1.)  // just print each 100 accepted model due to large amount of this
       {
      ...
      //  fprintf(stderr,
      //       "\n[ACCEPT CHECK before paras.push_back] npara=%d | nparav=%d | npara_tt=%d | npara_tr=%d | npara_vpvs=%d\n",
      //       para1.npara,
      //       para1.nparav,
      //       para1.npara_tt,
      //       para1.npara_tr,
      ...
      //   for (int jj = 0; jj < para1.npara; jj++)
      //   {
      //       fprintf(stderr,
      //           "  para1[%03d] = %g\n",
      //           jj,
...
double get_vp_1(modeldef tmodel, double tdep, int n_group)
{
    if (tdep < 0.) return -1.;
    ...
    if (tdep >= d2 && tdep < d1)
    {
        ...
        if (k < 0) k = 0;
        if (k >= (int)tmodel.groups[n_group].value1p.size())
            k = (int)tmodel.groups[n_group].value1p.size() - 1;

        return tmodel.groups[n_group].value1p[k];
    }
