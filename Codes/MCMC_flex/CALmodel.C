...
int readmod(modeldef &model, const char *name)
{
    ...
    // First pass: count groups and initialize defaults
    mff.open(name);
    if (!mff.is_open())
    {
    ...
    while (getline(mff, line))
    {
        ...
        model.groups[iid].flag  = atoi(v[1].c_str());
        model.groups[iid].thick = atof(v[2].c_str());

        tnp = atoi(v[3].c_str());
        model.groups[iid].np = tnp;

        model.groups[iid].ddep = atof(v[4].c_str());

        // flags are always stored in the final 11 columns
        int tmp_r_flag = atoi(v[size - 11].c_str());
        int tmp_q_flag = atoi(v[size - 10].c_str());
        int tmp_p_flag = atoi(v[size - 9].c_str());

        // =========================
        // Check model format
        // =========================
        if (model.groups[iid].flag == 4 && tnp != 2)
        {
        ...
        {
            if (tmp_p_flag == 5)
                expected_size = 5 + 3 * tnp + 11; // value + ratio + vpvs + control
            else
                expected_size = 5 + 2 * tnp + 11; // value + ratio + control
        }
        else if (model.groups[iid].flag == 2) // Bspline
        {
            if (tmp_p_flag == 5)
                expected_size = 5 + 2 * tnp + 11; // value + vpvs + control
            else
                expected_size = 5 + tnp + 11;     // value + control
        }
        else if (model.groups[iid].flag == 5) // water
        {
            if (tnp != 1)
            {
            ...
            expected_size = 5 + tnp + 11;
        }

        if (expected_size > 0 && size != expected_size)
        {
            cout << "#########wrong input for model: " << line
                 << "\t\t size: " << size
                 << " expected: " << expected_size
                 << " flag: " << model.groups[iid].flag
                 << " p_flag: " << tmp_p_flag
                 << endl;
            exit(0);
        }
        ...
        // Clear vectors
        // =========================
        model.groups[iid].value.clear();      // Vs / velocity values
        model.groups[iid].ratio.clear();      // thickness ratios
        model.groups[iid].value1vpvs.clear(); // explicit Vp/Vs values

        // =========================
        // Read Vs / velocity values
        // =========================
        for (i = 0; i < tnp; i++)
        {
            model.groups[iid].value.push_back(
                atof(v[5 + i].c_str())
            );
        }

        // =========================
        // Read thickness ratios for layered model
        // =========================
        if (model.groups[iid].flag == 1)
        {
            for (i = 0; i < tnp; i++)
            {
                model.groups[iid].ratio.push_back(
                    atof(v[5 + tnp + i].c_str())
                );
            }
        }

        // =========================
        // Read explicit Vp/Vs values if p_flag == 5
        // =========================
        if (tmp_p_flag == 5)
        {
            int vpvs_start;
            ...
            if (model.groups[iid].flag == 1)
                vpvs_start = 5 + 2 * tnp; // after value + ratio
            else
                vpvs_start = 5 + tnp;     // after value only

            for (i = 0; i < tnp; i++)
            {
                model.groups[iid].value1vpvs.push_back(
                    atof(v[vpvs_start + i].c_str())
                );
            }

            fprintf(stderr,
                    "[readmod] group=%d p_flag=5 read value1vpvs size=%zu\n",
                    iid,
                    model.groups[iid].value1vpvs.size());
        }

        // =========================
        // Read control values
        // final 11 columns:
        // r_flag q_flag p_flag vpvs vpvs1 drho drho1 dvs dvs1 fdvs fdvs1
        // =========================
        ...
        model.groups[iid].p_flag = tmp_p_flag;
        model.groups[iid].q_flag = tmp_q_flag;
        model.groups[iid].r_flag = tmp_r_flag;
    ...
    if (model.ngroup > 1)
    {
        cout << "flag0: " << model.groups[0].flag
             << "   flag1: " << model.groups[1].flag << endl;
    }
    ...
    */
    ...
    for(i=0;i<model.ngroup;i++)
    {
      ...
      // from each group, reading the value and join them into 1 layerized model
      {
        ...
        // fprintf(stderr,"%d | vs: %f | vp: %f | vpvs: %f\n",i,tvalue,tvaluep,tvpvs);
        // usleep(100000);
...
//-----------------------------------------------------	
    ...
    for (i = 0; i < (int)model.groups.size(); i++)
    {
        // id flag thick np ddep
        fprintf(ff, "%d %d %lf %d %lf ",
                i,
                model.groups[i].flag,
                model.groups[i].thick,
                model.groups[i].np,
                model.groups[i].ddep);
        ...
        // ratio values for layered model
        if (model.groups[i].flag == 1)
        {
            for (j = 0; j < model.groups[i].np; j++)
            {
        ...
        // explicit Vp/Vs values for p_flag == 5
        if (model.groups[i].p_flag == 5)
        {
            for (j = 0; j < model.groups[i].np; j++)
            {
        ...
        // final 11 control values
        fprintf(ff, "%d %d %d %g %g %g %g %g %g %g %g\n",
                model.groups[i].r_flag,
                model.groups[i].q_flag,
                model.groups[i].p_flag,
                model.groups[i].vpvs,
                model.groups[i].vpvs1,
                model.groups[i].drho,
                model.groups[i].drho1,
                model.groups[i].dvs,
                model.groups[i].dvs1,
                model.groups[i].fdvs,
                model.groups[i].fdvs1);
    }
...
// ===================== CRUST ONLY: Vs monotonic increase =====================
  ...
  // }
// ==========================================================================================================
// Check mantle layer is increasing Vs and Vp with depth
// ===================== MANTLE ONLY: Vs + Vp monotonic increase =====================
...
for (int ig = 0; ig < model.ngroup; ++ig)
{
  ...
  const std::vector<double> &vs = model.groups[ig].value1;
  const std::vector<double> &vp = model.groups[ig].value1p;
  ...
  if (vs.size() >= 2)
  {
    for (size_t k = 1; k < vs.size(); ++k)
    {
      ...
      {
        fprintf(stderr,
                "MANTLE Vs not monotonic within group! group=%d k=%zu  %.6f > %.6f\n",
                ig, k, vs[k - 1], vs[k]);
        return 0;
      }
    }
  ...
  if (vp.size() >= 2)
  {
    for (size_t k = 1; k < vp.size(); ++k)
    {
      if (vp[k] + eps < vp[k - 1])
      {
        fprintf(stderr,
                "MANTLE Vp not monotonic within group! group=%d k=%zu  %.6f > %.6f\n",
                ig, k, vp[k - 1], vp[k]);
        return 0;
      }
    }
...
// Check negative jump BETWEEN consecutive mantle groups
for (int ig = 1; ig < model.ngroup; ++ig)
{
  if (model.groups[ig - 1].q_flag == 4 && model.groups[ig].q_flag == 4)
  {
    if (!model.groups[ig - 1].value1.empty() &&
        !model.groups[ig].value1.empty())
    {
      double vs_prev = model.groups[ig - 1].value1.back();
      double vs_curr = model.groups[ig].value1.front();

      if (vs_curr + eps < vs_prev)
      {
        fprintf(stderr,
                "MANTLE Vs negative jump between mantle groups! g%d->g%d  %.6f > %.6f\n",
                ig - 1, ig, vs_prev, vs_curr);
        return 0;
      }
    }

    if (!model.groups[ig - 1].value1p.empty() &&
        !model.groups[ig].value1p.empty())
    {
      double vp_prev = model.groups[ig - 1].value1p.back();
      double vp_curr = model.groups[ig].value1p.front();

      if (vp_curr + eps < vp_prev)
      {
        fprintf(stderr,
                "MANTLE Vp negative jump between mantle groups! g%d->g%d  %.6f > %.6f\n",
                ig - 1, ig, vp_prev, vp_curr);
        return 0;
      }
    }
  }
...
// ==========================================================================================================
    // end of check crust-mantle layer is increasing Vp + Vs  with depth
// ===========================================================================================================
...
// ******************************************** //
      ...
      {   // abs vel
        ...
        for (j=0;j<model.groups[i].nlay;j++)
        {
          // fprintf(stderr,"Value check | group i=%d | lay j=%d | Vs=%g | Vp=%g\n",i,j,model.groups[i].value1[j],model.groups[i].value1p[j]);
          // usleep(1000000);
          // if ((model.groups[i].value1[j] > 4.9)) 
          if ((model.groups[i].value1[j] > 4.9) || (model.groups[i].value1[j] < 0.1) || (model.groups[i].value1p[j] < 0.1) || (model.groups[i].value1p[j] > 8.5))
        {
          cout<<"4.9 crit... i = "<<i<<" model.groups[i].value1[j]= "<<model.groups[i].value1[j]<<endl;
          fprintf(stderr,"Non-mantle velocity is invalid !!! |  i = %d | j = %d | Vs = %g | Vp = %g\n",i,j,model.groups[i].value1[j],model.groups[i].value1p[j]);
          return 0;
          }
        } 
        }
...
// ******************************************** //
        ...
        for(i=1;i<model.ngroup-1;i++)//between sed and crust layers only
		    {
        ...
        {
          if (model.groups[i].q_flag == 4 && model.groups[i-1].q_flag == 3)
          {
            if (model.groups[i].value1.empty()  || model.groups[i-1].value1.empty()  ||
                model.groups[i].value1p.empty() || model.groups[i-1].value1p.empty() ||
                model.groups[i].value1r.empty() || model.groups[i-1].value1r.empty())
            {
              fprintf(stderr,"Empty crust/mantle velocity vector in goodmodel!\n");
              return 0;
            }

            double crust_vs  = model.groups[i-1].value1.back();
            double mantle_vs = model.groups[i].value1.front();

            double crust_vp  = model.groups[i-1].value1p.back();
            double mantle_vp = model.groups[i].value1p.front();

            double crust_rho  = model.groups[i-1].value1r.back();
            double mantle_rho = model.groups[i].value1r.front();

            if (mantle_vs + eps < crust_vs // Vs
                ||     
                mantle_vp + eps < crust_vp // Vp
                ||  
                mantle_rho + eps < crust_rho   // Density
              )
            {
              fprintf(stderr,
                      "Bottom crust velocity greater than mantle first layer!!! "
                      "g%d->g%d | Vs %.6f -> %.6f | Vp %.6f -> %.6f | Rho %.6f -> %.6f\n",
                      i-1, i,
                      crust_vs, mantle_vs,
                      crust_vp, mantle_vp,
                      crust_rho, mantle_rho);
              return 0;
            }
          }
        }
