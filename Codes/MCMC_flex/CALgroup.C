...
//----------------------------------------------------- 
        double get_vp(double ts, int flag, double vpvs) 
          ...
          // flag: 3 vpvs = vpvs
          // flag: 4 tp = ts*(1.373+2.022/ts);
          // flag: 5 (same flag 3 but use vpvs pertubation values)
          if (flag==1) { 
          ...
          else if (flag==4) tp = ts*(1.373+2.022/ts);
          else if (flag==5) tp = ts*vpvs;
          else tp = ts*1.75;
          
          // fprintf(stderr,"flag=%d | Vs=%g | Vp=%g | ratio=%g\n",flag, ts, tp, vpvs);
          // usleep(100000);
          return tp;
          }
//-----------------------------------------------------
...
int updategroup1(groupdef &group)  // layer the model from layered model
{
    ...
    int r_flag = group.r_flag;
    int p_flag = group.p_flag;
    ...
    for (int i = 0; i < group.np; ++i) {
        ...
        // New Vp/Vs parameter override.
        // One Vp/Vs value for each original group.np control/sub-layer.
        ...
        // fprintf(stderr, "i = %d | Vs = %f | VpVs = %f | Rho = %f\n", i, ts, tvpvs, tdrho);
        // sleep(1);
    }
    ...
    group.value1p.clear();
    group.value1r.clear();
    // group.value1vpvs.clear(); // DO NOT clear this; it stores original Vp/Vs control values
    ...
    for (int i = 0; i < group.np; i++)
    {
        ...
        for (int k = 0; k < tn; k++) {
            ...
            group.value1vpvs.push_back(tvpvs);

            l_thick_check += sub_layer_thick;
            
            // fprintf(stderr,
            //     "updategroup1 small-layer | parent_i=%d | sub_k=%d/%d | final_layer=%d | "
            //     "thick=%g | Vs=%g | block_VpVs=%g | Vp=%g | p_flag=%d\n",
            //     i,k,tn,tn0,sub_layer_thick,ts_k,tvpvs,tp_k,group.p_flag);
            // usleep(100000);
        }
