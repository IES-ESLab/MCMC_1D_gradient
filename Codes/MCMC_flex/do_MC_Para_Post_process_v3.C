...
// Re-estimate the lower and higher bound. Uncommend when use
...
//output percent of models in 0.1km/s from min to max
  ...
  else
  {
    ...
    minmisfit_disp_o = kai_disp[index_range_disp];
    if (minmisfit_rf_o == 0)
    {
  ...
  {
  size_t index_range = static_cast<size_t>((kai_joint.size()));
  fprintf(stderr,"index_range %d \n",index_range);
    // Create another vector to copy sorted elements
    vector<float> arr(kai_joint.size());  // Allocate the same size for arr
    // Copy sorted vec into arr
  ...
  else
  {
    ...
    double maxmisfit_joint = *std::min_element(kai_joint.begin(),kai_joint.end());
    kai_cri = 2*maxmisfit_joint;
  }
  ...
  // HVL: 20250107 fix the wrong detection of minimum parameters
  
  n_min_rf = 0; //start from 0 cuz we scaled the array from min to max
  n_min_disp = 0; //start from 0 cuz we scaled the array from min to max
  n_min = 0; //start from 0 cuz we scaled the array from min to max
  ...
  for (i=0;i<nall;i++) 
  {
     switch (option) 
     {
      ...
      if (kai_joint[i] < kai_cri and kai_disp[i] <= minmisfit_disp_o and kai_rf[i] <= minmisfit_rf_o and signs1[i] > 0) 
      {
        ...
        if (kai_disp[i] < minmisfit_disp_new) 
        {
  ...
  cout<<"now let's do this! "<<j<<endl;

  vector<double> d1s,d2s,d3s,d4s,d5s,d6s,t_ds;
  double td1,td2,td3,td4,td5,td6,temp_d,temp_v,temp_v_min,temp_v_max,temp_v_mean,temp_v_std;
  double temp_v_p1std, temp_v_n1std;
  // double dstep;
  int ii,jj,kk,NN;
  ...
  for (i=0;i<model.ngroup;i++) {
    ...
    if (i==0) {td1=0.; td3=0.;}
      ...
      td1 = 99999.;
      t_ds.clear();
      for (jj=0;jj<models1.size();jj++) { 
         temp_d = 0.;
         for (k=0;k<i;k++)
            temp_d = temp_d + models1[jj].groups[k].thick;
         t_ds.push_back(temp_d);
         if (td1 > temp_d) td1 = temp_d;
         }
      td3 = cv_mean(t_ds);
      td5 = cv_std(t_ds);
      }
    ...
    if (i==model.ngroup-1) 
    {
      fprintf(stderr,"Characterize layers of the final model with the depth-step of %g (km)\n",dstep);
      NN = int(td2/dstep); 
    }
    }
// =====================================================================================================
  vector<double> mins,maxs,means,stds;
  double t_min,t_max, t_mean, t_std;
  vector<double> t_values;
  vector<ave_mod_def> all_mods;

  // ===============================
  // Average Vs in uniform depth step
  // ===============================
  all_mods.clear();

  for (i=0;i<model.ngroup;i++) {
    means.clear(); stds.clear(); mins.clear(); maxs.clear();

    for (j=0;j<NN;j++) {
      temp_d = j*dstep;
      t_values.clear();

      for (k=0;k<models1.size();k++) {
        temp_v = get_v_1(models1[k],temp_d,i); // Vs
        if (temp_v>=0.) t_values.push_back(temp_v);
      }

      if (t_values.size() < 2) {
        t_max = -1.; t_min = -1.; t_mean = -1.; t_std = -1.;
      }
      else {
        t_min = cv_min(t_values);
        t_max = cv_max(t_values);

        if (temp_d >= d3s[i]-d5s[i] && temp_d <= d4s[i]+d6s[i]) {
          t_mean = cv_mean(t_values);
          t_std  = cv_std(t_values);
        }
        else {
          t_mean = -1.;
          t_std  = -1.;
      ...
      means.push_back(t_mean);
      stds.push_back(t_std);
      mins.push_back(t_min);
      maxs.push_back(t_max);
    }

    ave_mod_def t_mod;
    t_mod.v_mins.clear();
    t_mod.v_maxs.clear();
    t_mod.max_thick = temp_d;
    t_mod.v_mins  = mins;
    t_mod.v_maxs  = maxs;
    t_mod.v_means = means;
    t_mod.v_stds  = stds;
    all_mods.push_back(t_mod);

    cout<<i<<"th Vs iteration ok!"<<endl;
  }

  fprintf(stderr,"now write average Vs model in uniform step!\n");
  ...
  for (i=0;i<NN;i++) {
    ...
    for (j=0;j<model.ngroup;j++) {
      if (temp_v_min > all_mods[j].v_mins[i] && all_mods[j].v_mins[i] > 0.)
        temp_v_min = all_mods[j].v_mins[i];

      if (temp_v_max < all_mods[j].v_maxs[i] && all_mods[j].v_maxs[i] > 0.)
        temp_v_max = all_mods[j].v_maxs[i];

      if (temp_v_n1std > (all_mods[j].v_means[i] - all_mods[j].v_stds[i]) &&
          all_mods[j].v_means[i] > 0.) {
        temp_v_n1std = all_mods[j].v_means[i] - all_mods[j].v_stds[i];
      }

      if (temp_v_p1std < (all_mods[j].v_means[i] + all_mods[j].v_stds[i]) &&
          all_mods[j].v_means[i] > 0.) {
        temp_v_p1std = all_mods[j].v_means[i] + all_mods[j].v_stds[i];
      }

      if (temp_d >= d3s[j] && temp_d <= d4s[j]) {
        temp_v_mean = all_mods[j].v_means[i];
        temp_v_std  = all_mods[j].v_stds[i];
      }
    }

    fprintf(ff0,"%g %g %g %g %g %g %g\n",
            temp_d,temp_v_mean,temp_v_std,
            temp_v_min,temp_v_max,temp_v_n1std,temp_v_p1std);
  }
  ...
  // ===============================
  // Average Vp in uniform depth step
  // Same logic as Vs
  // ===============================
  all_mods.clear();
  ...
  for (i=0;i<model.ngroup;i++) {
    ...
    for (j=0;j<NN;j++) {
      ...
      for (k=0;k<models1.size();k++) {
        temp_v = get_vp_1(models1[k],temp_d,i); // Vp
        if (temp_v>=0.) t_values.push_back(temp_v);
      }
    ...
    cout<<i<<"th Vp iteration ok!"<<endl;
  }

  fprintf(stderr,"now write average Vp model in uniform step!\n");

  sprintf(minname,"%s/%s.ave.vp.mod",outdir,wmodelname1);
