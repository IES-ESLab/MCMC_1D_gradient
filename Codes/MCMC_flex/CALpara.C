...
//public:
    ...
    // sleep(1);;
	  ...
	  para.npara_vpvs=0; // nuber of vpvs perturbation
	  para.L=0.;
	  para.misfit=0.;
	  para.flag=0;
	  return 1;
...
//-----------------------------------------------------	 
	...
	{
		/*
		Read the in.para_STA file

		Parameter type:
		0  = Vs / velocity perturbation
		1  = thickness perturbation
		-1 = Vp/Vs perturbation
		*/
		...
		if (!mff.is_open())
		{
			cout << "########para file " << fname << " does not exist!\n exit!!\n";
			exit(0);
		}
		...
		while(getline(mff,line))
		{
			...
			int ptype = std::atoi(v[0].c_str());

			if (ptype == 0)
			{
				// Vs / velocity perturbation
				nparav++;
			}
			else if (ptype == 1)
			{
				...
				if (v.size() == 5)
				{
					// total thickness perturbation
					npara_tt++;
				}
				else if (v.size() > 5)
				{
					// thickness-ratio perturbation 
					npara_tr++;
				}
			}
			else if (ptype == -1)
		...
		para.npara = k;
		para.nparav = nparav;
		para.npara_tt = npara_tt;
		para.npara_tr = npara_tr;
		para.npara_vpvs = npara_vpvs;

		fprintf(stderr," readpara > npara: %d | npara velocity: %d | npara total thickness: %d | npara thickness ratio: %d | npara vpvs: %d\n",k, nparav, npara_tt, npara_tr, npara_vpvs);
...
int gen_newpara(paradef &inpara, paradef &outpara, int pflag, int invtype)
{
	...
	// Modification for the case use of vpvs perturbation
    int i, flag, ibad;
    ...
    int idx_v0    = 0;
    int idx_tt0   = inpara.nparav;
    int idx_tr0   = inpara.nparav + inpara.npara_tt;
    int idx_vpvs0 = inpara.nparav + inpara.npara_tt + inpara.npara_tr;

    int n_v    = inpara.nparav;
    int n_tt   = inpara.npara_tt;
    int n_tr   = inpara.npara_tr;
    int n_vpvs = inpara.npara_vpvs;

    if (idx_vpvs0 + n_vpvs != inpara.npara)
    {
        fprintf(stderr,
            "bad parameter indexing: npara=%d nparav=%d npara_tt=%d npara_tr=%d npara_vpvs=%d idx_vpvs0=%d\n",
            inpara.npara, n_v, n_tt, n_tr, n_vpvs, idx_vpvs0
        );
        return 0;
    }

    // ============================================================
    // pflag == 0: uniform generation
    // ============================================================
    if (pflag == 0)
    {
        // -------- Vs parameters --------
        for (i = idx_v0; i < idx_v0 + n_v; i++)
        {
            cvr = gen_random_unif01();
            newv = cvr * (inpara.space1[i][1] - inpara.space1[i][0])
                 + inpara.space1[i][0];

            if (newv > inpara.space1[i][1] || newv < inpara.space1[i][0])
            {
                fprintf(stderr,
                    "bad Vs random: i=%d cvr=%g newv=%g min=%g max=%g\n",
        ...
        // -------- total thickness parameters --------
        for (i = idx_tt0; i < idx_tt0 + n_tt; i++)
        {
            cvr = gen_random_unif01();
            newv = cvr * (inpara.space1[i][1] - inpara.space1[i][0])
                 + inpara.space1[i][0];
            ...
            {
                fprintf(stderr,
                    "bad total thickness random: i=%d cvr=%g newv=%g min=%g max=%g\n",
                    i, cvr, newv, inpara.space1[i][0], inpara.space1[i][1]
                );
                return 0;
            }
        ...
        // -------- thickness-ratio parameters --------
        if (n_tr > 1)
        {
            std::vector<double> w(n_tr);

            for (int k = 0; k < n_tr; ++k)
                w[k] = gen_random_unif01();

            if (!perturb_thickness_ratios_inplace_weighted(
                    outpara.parameter, idx_tr0, n_tr, w))
            {
                return 0;
            }
        }
        ...
        for (i = idx_vpvs0; i < idx_vpvs0 + n_vpvs; i++)
        {
            ...
            {
                fprintf(stderr,
                    "bad VpVs random: i=%d cvr=%g newv=%g min=%g max=%g\n",
                    i, cvr, newv, inpara.space1[i][0], inpara.space1[i][1]
                );
                return 0;
            }
        ...
        for (i = idx_v0; i < idx_v0 + n_v; i++)
        {
            ...
            while (flag < 1)
            {
                ...
                if (newv > inpara.space1[i][1] || newv < inpara.space1[i][0])
                {
                    ...
                    {
                        fprintf(stderr,
                            "cannot find good Vs random: i=%d old=%g mean=%g sigma=%g newv=%g min=%g max=%g ibad=%d\n",
                            i, inpara.parameter[i], mean, sigma, newv,
                            inpara.space1[i][0], inpara.space1[i][1], ibad
                        );
                        exit(1);
                    }
                    continue;
        ...
        for (i = idx_tt0; i < idx_tt0 + n_tt; i++)
        {
            ...
            while (flag < 1)
            {
                ...
                if (newv > inpara.space1[i][1] || newv < inpara.space1[i][0])
                {
                    ...
                    {
                        fprintf(stderr,
                            "cannot find good thickness random: i=%d old=%g mean=%g sigma=%g newv=%g min=%g max=%g ibad=%d\n",
                            i, inpara.parameter[i], mean, sigma, newv,
                            inpara.space1[i][0], inpara.space1[i][1], ibad
                        );
                        exit(1);
        ...
        for (i = idx_vpvs0; i < idx_vpvs0 + n_vpvs; i++)
        {
            ...
            while (flag < 1)
            {
                ...
                if (newv > inpara.space1[i][1] || newv < inpara.space1[i][0])
                {
                    ...
                    {
                        fprintf(stderr,
                            "cannot find good VpVs random: i=%d old=%g mean=%g sigma=%g newv=%g min=%g max=%g ibad=%d\n",
                            i, inpara.parameter[i], mean, sigma, newv,
                            inpara.space1[i][0], inpara.space1[i][1], ibad
                        );
                        exit(1);
	...
	// // // ================= Print new parameters for checking =================
	// fprintf(stderr, "\n========== gen_newpara output ==========\n");
	// fprintf(stderr,
	// 	"npara=%d | nparav=%d | npara_tt=%d | npara_tr=%d | npara_vpvs=%d\n",
	// 	outpara.npara,
	// 	outpara.nparav,
	// 	outpara.npara_tt,
	// 	outpara.npara_tr,
	...
	// 		ptype = "Vs";
	// 	else if (i < idx_tr0)
	// 		ptype = "TotalThickness";
	// 	else if (i < idx_vpvs0)
	// 		ptype = "ThicknessRatio";
	// 	else
	// 		ptype = "VpVs";

	// 	fprintf(stderr,
	// 		"para[%03d] %-15s value=%12.6f  min=%12.6f  max=%12.6f  sigma=%12.6f\n",
	// 		i,
	// 		ptype,
	// 		outpara.parameter[i],
	// 		outpara.space1[i][0],
...
//-----------------------------------------------------	 
	...
	// ========================================================
      ...
      for(i=0;i<inpara.npara;i++)
	  {
		...
		{
			nv = atoi(inpara.para0[i][5].c_str());
...
//-----------------------------------------------------	 
    ...
    // sleep(1);
	  ...
	  for(int i=0;i<para.npara;i++)
	  {
		...
		// else if (p0 == -1) // Vp/Vs perturbation per sublayer
		// {
		// 	nv = atoi(para.para0[i][5].c_str());
		...
		else if (p0 == -1) // Vp/Vs perturbation per sublayer
		{
			...
			if ((int)outmodel.groups[ng].value1vpvs.size() <= nv)
			{
				outmodel.groups[ng].value1vpvs.resize(
					nv + 1,
					outmodel.groups[ng].vpvs
				);
			}

			outmodel.groups[ng].value1vpvs[nv] = newv;

			// fprintf(stderr,"para2mod VpVs: i=%d group=%d nv=%d value=%g size=%zu\n",i, ng, nv, newv, outmodel.groups[ng].value1vpvs.size());
			// usleep(1000000);
		}
		else if(p0==-2) 
...
//};
	int get_misfit1(paradef &para, modeldef &outmodel, double p, int invtype)
	{
		...
		// this function caused the reset of the Vpvs parameters
		mod2para(outmodel, temppara, para, invtype);
