...
"""
Combine 2 PNG figures side-by-side:
...
------
1) Map figure:
   output_figures/station_maps/map_{station}.png

2) Current MCMC figure:
   output_figures/1D_model/MCMC_{station}.png

Output
------
output_figures/QC_map_mcmc/QC_{station}.png

Notes
-----
- Pairing is by station name string.
- Map filename format is:
      map_{station}.png
  example:
      map_121.075-25.025.png
- Adds text labels above the 2 panels.
"""
...
MAP_DIR = Path(PWD) / "output_figures" / "station_maps"
...
def extract_station_from_map(fname: str) -> Optional[str]:
    ...
    Example:
        map_121.075-25.025.png
    """
    if not fname.startswith(MAP_PREFIX) or not fname.endswith(PNG_SUFFIX):
        return None
    return fname[len(MAP_PREFIX):-len(PNG_SUFFIX)]
...
def combine_pair(map_png: Path, curr_png: Path, out_png: Path):
    ...
    x_map = SIDE_PAD_PX
    x_curr = x_map + im_map.width + GAP_PX
    y_img = TOP_PAD_PX
...
def _worker(payload):
    map_png, curr_png, out_png = payload
    combine_pair(map_png, curr_png, out_png)
    return str(out_png)
...
def main():
    ...
    maps = {}
    for p in MAP_DIR.glob(f"{MAP_PREFIX}*{PNG_SUFFIX}"):
    ...
    common_stations = set(maps) & set(currs)
    ...
    print("Matched pairs     :", len(common_stations_sorted))
    ...
    for sta in common_stations_sorted:
        ...
        jobs.append((maps[sta], currs[sta], out_png))

    if not jobs:
        print("\nNo matched map-MCMC pairs found.")
        return
