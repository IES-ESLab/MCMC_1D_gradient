...
def read_posterior_vs_and_depth_with_depthfile(value_file, depth_file, plot_type):
    with open(value_file, "r") as ff:
        posterior_v_tmp = [x.strip().split() for x in ff if x.strip()]

    with open(depth_file, "r") as ff:
        posterior_depth_tmp = [x.strip().split() for x in ff if x.strip()]

    if len(posterior_v_tmp) != len(posterior_depth_tmp):
        die(
            "Inconsistent posterior models: value rows={} depth rows={}".format(
                len(posterior_v_tmp), len(posterior_depth_tmp)
            )
        )
    ...
    for i, (v_row, dep_row) in enumerate(zip(posterior_v_tmp, posterior_depth_tmp)):
        if len(v_row) != len(dep_row):
            die(
                "Inconsistent point count in model #{}: value={} depth={}".format(
                    i, len(v_row), len(dep_row)
                )
            )

        v = np.array([float(x) for x in v_row], dtype=float)
        dep = np.array([float(x) for x in dep_row], dtype=float)

        mask = np.isfinite(v) & np.isfinite(dep)
        v = v[mask]
        dep = dep[mask]
        ...
        posterior_v.append(v)
        posterior_depth.append(dep)

    return posterior_v, posterior_depth
...
def build_paths(sta, mdir, pwd):
    ...
    paths = {
        ...
        "velmodfile": os.path.join(project_root, "Vel_mod", "NTW1d_H14_intp_50km"),
        ...
        "avemod_vp_file": os.path.join(sta_dir, "MC.{}.ave.vp.mod".format(sta)),
...
def main():
    ...
    posteriorVs, posteriordepth = read_posterior_vs_and_depth_with_depthfile(paths["posteriorfile"],paths["posteriorfile"] + "_depth", plot_type)

    posteriorVp = None
    posteriorVpdepth = None

    if os.path.isfile(paths["posteriorfile_vp"]) and os.path.isfile(paths["posteriorfile"] + "_depth"):
        posteriorVp, posteriorVpdepth = read_posterior_vs_and_depth_with_depthfile(
            paths["posteriorfile_vp"],
            paths["posteriorfile"] + "_depth",
            plot_type
        )
    else:
        print("[WARN] No posterior Vp/depth file found")
    ...
    if os.path.isfile(paths["avemod_vp_file"]):
        avgvpdepthnew, avgvpnew, avgvpstd, avgvpstdn, avgvpstdp = np.loadtxt(
            paths["avemod_vp_file"], usecols=(0, 1, 2, 5, 6), unpack=True
        )

        with open(paths["out_avg_vp_txt"], "w") as fobj:
            for i in range(len(avgvpdepthnew)):
                fobj.write("{} {}\n".format(avgvpdepthnew[i], avgvpnew[i]))
    else:
        print("[WARN] No average Vp model found: {}".format(paths["avemod_vp_file"]))
    ...
    if posteriorVp is not None:
        for jj in range(len(posteriorVp) - 1):
            ax1.plot(
                posteriorVp[jj], posteriorVpdepth[jj],
                c="lightblue", alpha=0.10, lw=2
            )
    ...
    if avgvpnew is not None:
        ax1.plot(
            avgvpnew, avgvpdepthnew,
            "co--", mfc="w", lw=2, ms=10,
            label="Final Vp", zorder=7, mec="k"
        )
        ax1.errorbar(
            avgvpnew, avgvpdepthnew,
            xerr=avgvpstd,
            fmt="-o", ecolor="c",
            elinewidth=2, capsize=4, alpha=1.0
        )

    # Average Vp/Vs ratio on bottom-left full-depth panel
    # if avgvpnew is not None:
    # Average Vp/Vs ratio with standard deviation
    nratio = min(
        len(avgdepthnew), len(avgvpdepthnew),
        len(avgvsnew), len(avgvpnew),
        len(avgnewstd), len(avgvpstd)
    )

    ratio_depth = avgdepthnew[:nratio]
    ratio_vs = avgvsnew[:nratio]
    ratio_vp = avgvpnew[:nratio]
    ratio_vs_std = avgnewstd[:nratio]
    ratio_vp_std = avgvpstd[:nratio]

    ratio_mask = (
        np.isfinite(ratio_depth) &
        np.isfinite(ratio_vs) &
        np.isfinite(ratio_vp) &
        np.isfinite(ratio_vs_std) &
        np.isfinite(ratio_vp_std) &
        (ratio_vs > 0.0) &
        (ratio_vp > 0.0)
    )

    ratio_depth = ratio_depth[ratio_mask]
    ratio_vs = ratio_vs[ratio_mask]
    ratio_vp = ratio_vp[ratio_mask]
    ratio_vs_std = ratio_vs_std[ratio_mask]
    ratio_vp_std = ratio_vp_std[ratio_mask]

    ratio_vpvs = ratio_vp / ratio_vs

    # error propagation:
    # R = Vp / Vs
    # sigma_R = R * sqrt((sigma_Vp/Vp)^2 + (sigma_Vs/Vs)^2)
    ratio_vpvs_std = ratio_vpvs * np.sqrt(
        (ratio_vp_std / ratio_vp) ** 2 +
        (ratio_vs_std / ratio_vs) ** 2
    )

    ax1.plot(
        ratio_vpvs, ratio_depth,
        "k^-", lw=2.0, ms=7,
    ...
    ax1.set_xlabel("Velocity (km/s)", fontdict=FONT_LABEL)
    ...
    if avgvpnew is not None:
        ...
        # Average Vp/Vs ratio with standard deviation for zoom-in panel
        ax4.plot(
            ratio_vpvs, ratio_depth,
            "k^-", lw=2.0, ms=7,
            label="Final Vp/Vs", zorder=8
    ...
    ax4.set_xlabel("Velocity (km/s)", fontdict=FONT_LABEL)
